...
void SetCaretShap(HWND DialogHWND,ULONG CtrlID)
{
	...
	txr.lpstrText = &aChar;
	SendDlgItemMessage(DialogHWND,CtrlID,WM_GETTEXT,10,(ULONG)tem);
	aChar = tem[0];//tem[SelStart-1];
							 ...
							 IN LPARAM LParam)  
{
	...
	ParentHwnd = (HWND)GetWindowLong(WindowHandle,GWL_HWNDPARENT);
	switch (Message) { 
	case WM_RBUTTONDOWN:
...
{	
	...
	{
		SetWindowLong(GetDlgItem(hWnd,CtrlID),GWL_USERDATA,0);
		SetWindowLong(GetDlgItem(hWnd,CtrlID),GWL_WNDPROC,(LONG)OldEditWndProc);
		SetDlgItemText(hWnd,CtrlID,"");	
	}
	else if( InputMode == 1 )//十六进制模式
	{	
		OldEditWndProc = (WNDPROC)SetWindowLong(GetDlgItem(hWnd,CtrlID),GWL_WNDPROC,(LONG)NewEditWndProc);
		SetWindowLong(GetDlgItem(hWnd,CtrlID),DWL_USER,(LONG)CtrlID);
		SetDlgItemText(hWnd,CtrlID,"");
	}
}