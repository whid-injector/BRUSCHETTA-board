//
//   ch347eeprom programmer version 0.6 (Beta)
//
...
int main(int argc, char **argv)
{
    ...
    char operation = 0;
    uint32_t speed = ch347_I2C_STANDARD_SPEED;
    uint8_t *verifybuf;
    ...
    while (true) {
        ...
        case 'p':
            if (strstr(optarg, "low"))
                speed = ch347_I2C_LOW_SPEED;
            else if (strstr(optarg, "fast"))
                speed = ch347_I2C_FAST_SPEED;
            else if (strstr(optarg, "high"))
                speed = ch347_I2C_HIGH_SPEED;
            else
                speed = ch347_I2C_STANDARD_SPEED;
            break;
        case 'e':
            if (!operation)
                operation = 'e';
    ...
    if (!(devHandle = ch347configure(USB_LOCK_VENDOR, USB_LOCK_PRODUCT))) {
    ...
    if (ch347setstream(devHandle, speed) < 0) {
    ...
    case 'r': // read
        ...
        if (ch347readEEPROM(devHandle, readbuf, eepromsize, eeprom_info)
            < 0) {
    ...
    case 'w': // write
        ...
        if (ch347writeEEPROM(devHandle, readbuf, eepromsize, eeprom_info) < 0) {
